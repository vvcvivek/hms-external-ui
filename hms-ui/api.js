...
const SF_CONFIG = {
  clientId:     'YOUR_CONSUMER_KEY',   // ← Paste your Consumer Key here
...
const API = {
  ...
  async getDashboardStats() {
    const [patients, doctors, appts, invoices] = await Promise.all([
      this.query(`SELECT COUNT() FROM Patient__c WHERE Patient_Status__c = 'Active'`),
      this.query(`SELECT COUNT() FROM Doctor__c WHERE Doctor_Status__c = 'Active'`),
      this.query(`SELECT COUNT() FROM Appointment__c WHERE Appointment_Date__c = TODAY AND Status__c != 'Cancelled'`),
      this.query(`SELECT COUNT() FROM Invoice__c WHERE Payment_Status__c IN ('Unpaid','Partially Paid')`)
    ]);
    return {
      totalPatients:      patients[0].expr0,
      activeDoctors:      doctors[0].expr0,
      todayAppointments:  appts[0].expr0,
      pendingInvoices:    invoices[0].expr0
    };
  },
  ...
  async getOverdueInvoices() {
    return this.query(`
      SELECT Id, Name, Due_Date__c, Outstanding_Balance__c, Payment_Status__c,
        Patient__r.First_Name__c, Patient__r.Last_Name__c
      FROM Invoice__c
